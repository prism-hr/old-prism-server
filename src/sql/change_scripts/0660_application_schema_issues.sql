...
WHERE role_id IN ("APPLICATION_REFEREE", "APPLICATION_VIEWER_REFEREE")
;
...
INSERT INTO USER_ROLE (application_id, user_id, role_id, requesting_user_id, assigned_timestamp)
	...
	WHERE (APPLICATION.state_id LIKE "APPLICATION_VALIDATION%"
		OR APPLICATION.state_id LIKE "APPLICATION_REVIEW%"
		OR APPLICATION.state_id LIKE "APPLICATION_INTERVIEW%"
		OR APPLICATION.state_id LIKE "APPLICATION_APPROVAL%"
		OR APPLICATION.state_id = "APPLICATION_APPROVAL"
...
INSERT INTO USER_ROLE (application_id, user_id, role_id, requesting_user_id, assigned_timestamp)
	SELECT APPLICATION.id, APPLICATION_REFEREE.user_id, "APPLICATION_VIEWER_REFEREE", APPLICATION.user_id, APPLICATION.submitted_timestamp
	FROM APPLICATION INNER JOIN APPLICATION_REFEREE
		ON APPLICATION.id = APPLICATION_REFEREE.application_id
	LEFT JOIN USER_ROLE
...
INSERT INTO USER_NOTIFICATION_INDIVIDUAL (user_role_id, notification_template_id, last_notification_timestamp)
	SELECT USER_ROLE.id, "APPLICATION_PROVIDE_REFERENCE_REQUEST", DATE(APPLICATION_REFEREE.last_notified)
	FROM APPLICATION_REFEREE INNER JOIN USER_ROLE
		ON APPLICATION_REFEREE.application_id = USER_ROLE.application_id
		AND APPLICATION_REFEREE.user_id = USER_ROLE.user_id
	WHERE USER_ROLE.role_id = "APPLICATION_REFEREE"
		AND APPLICATION_REFEREE.last_notified IS NOT NULL
;
