    ...
    public void setup() {
        ...
        template.execute(new TransactionCallbackWithoutResult() {
            ...
            protected void doInTransactionWithoutResult(TransactionStatus status) {
                ...
                QualificationInstitution institution = new QualificationInstitutionBuilder().code("code").name("a18").domicileCode("AE").enabled(true).build();
                ...
                referee1 = new RefereeBuilder().declined(false).application(application1).lastNotified(lastNotified.toDate()).user(refereeUser1).reference(new ReferenceCommentBuilder().build()).build();
                referee2 = new RefereeBuilder().declined(false).application(application2).lastNotified(lastNotified.toDate()).reference(new ReferenceCommentBuilder().build()).user(refereeUser2).build();
                referee3 = new RefereeBuilder().declined(false).application(application1).lastNotified(lastNotified.toDate()).reference(new ReferenceCommentBuilder().build()).user(refereeUser3).build();
