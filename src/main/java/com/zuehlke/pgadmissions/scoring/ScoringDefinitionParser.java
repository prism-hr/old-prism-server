...
public class ScoringDefinitionParser {
    ...
    @Value("${custom.forms.xsd.location}")
    private String customFormsXsdLocation;

    @PostConstruct
    public void setup() throws Exception {
        File file = new DefaultResourceLoader().getResource(customFormsXsdLocation).getFile();
    ...
    public CustomQuestions parseScoringDefinition(String definitionXml) throws ScoringDefinitionParseException {
        CustomQuestions scoringDefinition = null;
        try {
            scoringDefinition = (CustomQuestions) unmarshaller.unmarshal(new StringReader(definitionXml));
        } catch (JAXBException e) {
            throw new ScoringDefinitionParseException(e.getLinkedException().getLocalizedMessage(), e);
        }
        validateScoringDefinition(scoringDefinition);
        return scoringDefinition;
    }

    public void validateScoringDefinition(CustomQuestions scoringDefinition) throws ScoringDefinitionParseException {
    ...
    private void validateDropDownType(Question question) throws ScoringDefinitionParseException {
        if (question.getType().equals(QuestionType.DROPDOWN)) {
            if (question.getOptions() == null) {
                throw new ScoringDefinitionParseException(String.format(OPTIONS_ELEMENT_MISSING_MESSAGE_TEMPLATE, question.getLabel()));
            }
        }
    }

    private void validateDateType(Question question) throws ScoringDefinitionParseException {
    ...
    private void validateDate(String date) throws ScoringDefinitionParseException {
        if (date != null && !date.matches(DATE_FORMAT) && !(date.equals(TODAY))) {
            throw new ScoringDefinitionParseException(String.format(MALFORMATTED_DATE_ERROR_MESSAGE_TEMPLATE, date));
        }
    }
