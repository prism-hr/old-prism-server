...
UPDATE APPLICATION_FUNDING INNER JOIN IMPORTED_ENTITY
	...
	AND IMPORTED_ENTITY.imported_entity_type_id = "FUNDING_SOURCE"
...
UPDATE APPLICATION_PERSONAL_DETAIL INNER JOIN IMPORTED_ENTITY
	...
	AND IMPORTED_ENTITY.imported_entity_type_id = "TITLE"
...
INSERT INTO IMPORTED_LANGUAGE_QUALIFICATION_TYPE
	SELECT NULL, 5243, "IELTS_ACADEMIC", "IELTS Academic", 4.00, 9.00, 4.00, 9.00, 4.00, 9.00, 4.00, 9.00, 4.00, 9.00, 1
		UNION
	SELECT NULL, 5243, "TOEFL_PAPER", "TOEFL iBT Paper", 0, 120, 0, 30, 0, 30, 0, 30, 0, 30, 1
		UNION
	SELECT NULL, 5243, "TOEFL_INTERNET", "TOEFL iBT Internet", 0, 120, 0, 30, 0, 30, 0, 30, 0, 30, 1
;
...
UPDATE APPLICATION_LANGUAGE_QUALIFICATION INNER JOIN IMPORTED_ENTITY
	ON APPLICATION_LANGUAGE_QUALIFICATION.language_qualification_type_id = IMPORTED_ENTITY.id
INNER JOIN IMPORTED_LANGUAGE_QUALIFICATION_TYPE
	ON IMPORTED_ENTITY.code = SUBSTRING(IMPORTED_LANGUAGE_QUALIFICATION_TYPE.code, 1, 5)
	AND IMPORTED_ENTITY.code = "TOEFL"
	AND IMPORTED_LANGUAGE_QUALIFICATION_TYPE.code = "TOEFL_PAPER"
SET APPLICATION_LANGUAGE_QUALIFICATION.language_qualification_type_id = IMPORTED_LANGUAGE_QUALIFICATION_TYPE.id
WHERE APPLICATION_LANGUAGE_QUALIFICATION.exam_online = 0
;
...
INNER JOIN IMPORTED_LANGUAGE_QUALIFICATION_TYPE
	ON IMPORTED_ENTITY.code = SUBSTRING(IMPORTED_LANGUAGE_QUALIFICATION_TYPE.code, 1, 5)
	AND IMPORTED_ENTITY.code = "TOEFL"
	AND IMPORTED_LANGUAGE_QUALIFICATION_TYPE.code = "TOEFL_INTERNET"
SET APPLICATION_LANGUAGE_QUALIFICATION.language_qualification_type_id = IMPORTED_LANGUAGE_QUALIFICATION_TYPE.id
WHERE APPLICATION_LANGUAGE_QUALIFICATION.exam_online = 1
;
...
UPDATE STATE INNER JOIN SCOPE
	ON STATE.id LIKE CONCAT(SCOPE.id, "%")
SET STATE.scope_id = SCOPE.id
;
