...
public class SftpAttachmentsSendingService {
     ...
     *            application form containing attachments to be sent
     * @throws CouldNotCreateAttachmentsPack
     *             problem with building zip-pack with attachments
     * @throws LocallyDefinedSshConfigurationIsWrong
     *             local problems with preparing ssh session (like - no access to key file etc.)
     * @throws CouldNotOpenSshConnectionToRemoteHost
     *             network is down, some firewall stopped us or SSH authentication failed
     * @throws SftpTargetDirectoryNotAccessible
     *             target directory does not exist or our SSH user has no permissions to access this directory
     * @return the name of the created ZIP file on the server
     * @throws SftpTransmissionFailedOrProtocolError
     *             we were able to establish SSH connection but SFTP transfer over this connection failed - protocol error happened (possibly we lost the
    ...
    public String sendApplicationFormDocuments(Application applicationForm) throws CouldNotCreateAttachmentsPack, LocallyDefinedSshConfigurationIsWrong,
        ...
        try {
            ...
            try {
                ...
                throw new CouldNotOpenSshConnectionToRemoteHost("Failed to open SSH connection to PORTICO host, configured address was: " + sftpHost + ":"
                        + sftpPort + " username/password=" + sftpUsername + "/" + sftpPassword, e);
            }
            ...
            try {
                ...
                throw new SftpTransmissionFailedOrProtocolError(
                        "Failed to open sftp channel over previously established SSH connection, remote host address is: " + sftpHost + ":" + sftpPort, e);
            }
            ...
            try {
                ...
                throw new SftpTargetDirectoryNotAccessible("Failed to access remote directory for SFTP transmission: " + targetFolder
                        + ", remote host address is: " + sftpHost + ":" + sftpPort, e);
            }
            ...
            try {
                // FIXME obtain export reference number from the comment
                String exportReferenceNumber = null; // applicationForm.getUclBookingReferenceNumber();
                String finalZipName = exportReferenceNumber + ".zip";
                sftpOs = sftpChannel.put(finalZipName, ChannelSftp.OVERWRITE);
                attachmentsZipCreator.writeZipEntries(applicationForm, exportReferenceNumber, sftpOs);
                return finalZipName;
            } catch (SftpException e) {
                throw new SftpTransmissionFailedOrProtocolError("SFTP protocol error during transmission of attachments for application form "
                        + applicationForm.getId(), e);
            } catch (IOException e) {
                throw new SftpTransmissionFailedOrProtocolError("SFTP protocol error during transmission of attachments for application form "
                        + applicationForm.getId() + "; possibly we lost the connection to the remote host", e);
            }
