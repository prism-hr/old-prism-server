	  ...
	  <select id="programReviewers" class="list-select-from" multiple="multiple" size="${avaliableOptionsSize}">
	    ...
	    <optgroup id="previous" label="Previous reviewers">
			...
			<#list applicationReviewers as reviewer>
				<option value="${applicationForm.applicationNumber}|${encrypter.encrypt(reviewer.id)}" class="selected" disabled="disabled">${reviewer.firstName?html} ${reviewer.lastName?html}</option>
			</#list>
			<#list pendingReviewers as unsaved>									
				<option value="${encrypter.encrypt(unsaved.id)}" class="selected" disabled="disabled">${unsaved.firstName?html} ${unsaved.lastName?html}</option>
			</#list>
	    </optgroup>
	  </select>
	</div>
