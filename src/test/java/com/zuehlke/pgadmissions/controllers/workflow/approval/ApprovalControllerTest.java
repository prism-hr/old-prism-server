...
public class ApprovalControllerTest {
	...
	public void shouldGetProgrammeSupervisors() {
		...
		controller = new ApprovalController(applicationServiceMock, userServiceMock, approvalServiceMock, approvalRoundValidatorMock, supervisorPropertyEditorMock) {
			@Override
	...
	public void shouldGetListOfPreviousSupervisorsAndAddReviewersWillingToApprovalRoundWitDefaultSupervisorsRemoved() {
		...
		InterviewComment interviewOne = new InterviewCommentBuilder().id(1).user(interviewerWillingToSuperviseOne).willingToSupervise(true).toInterviewComment();
		...
		InterviewComment interviewThree = new InterviewCommentBuilder().id(1).user(interviewerWillingToSuperviseTwo).willingToSupervise(true).toInterviewComment();
		
		

		final Program program = new ProgramBuilder().id(6).supervisors(defaultSupervisor).toProgram();

		final ApplicationForm applicationForm = new ApplicationFormBuilder().id(5).program(program).comments(interviewOne, interviewTwo, interviewThree).toApplicationForm();
		controller = new ApprovalController(applicationServiceMock, userServiceMock, approvalServiceMock, approvalRoundValidatorMock, supervisorPropertyEditorMock) {
			@Override
			public ApplicationForm getApplicationForm(String applicationId) {
				if (applicationId == "5") {
		...
		EasyMock.expect(userServiceMock.getAllPreviousSupervisorsOfProgram(program)).andReturn(Arrays.asList(previousSupervisor, defaultSupervisor, interviewerWillingToSuperviseOne));
	...
	public void shouldReturnNewApprovalRoundWithExistingRoundsSupervisorsIfAny() {
		...
		final ApplicationForm application = new ApplicationFormBuilder().id(2).applicationNumber("abc").latestApprovalRound(new ApprovalRoundBuilder().supervisors(supervisorOne, suprvisorTwo).toApprovalRound()).toApplicationForm();
	...
	public void shouldReturnApprovalRoundWithWillingToApprovalRoundWithSupervisorsOfPreviousApprovalRoundRemoved() {
		...
		final ApplicationForm application = new ApplicationFormBuilder().id(2).applicationNumber("abc").comments(interviewOne, interviewTwo, interviewThree).latestApprovalRound(new ApprovalRoundBuilder().supervisors(interviewerOne, interviewerTwo).toApprovalRound()).toApplicationForm();
	...
	public void shouldNotSaveApprovalRoundAndReturnToApprovalPageIfHasErrors() {
		...
		ApprovalRound interview = new ApprovalRoundBuilder().application(applicationForm).toApprovalRound();
		...
		assertEquals("/private/staff/supervisors/supervisors_section", controller.moveToApproval("abc", interview, errorsMock));
