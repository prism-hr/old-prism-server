...
INSERT INTO COMMENT (program_id, action_id, user_id, role_id, created_timestamp, transition_state_id)
	...
	FROM PROGRAM INNER JOIN OPPORTUNITY_REQUEST
		...
		UNION
	SELECT PROGRAM.id, "PROGRAM_COMPLETE_APPROVAL_STAGE", 2048, "SYSTEM_ADMINISTRATOR", MAX(OPPORTUNITY_REQUEST_COMMENT.created_timestamp), "PROGRAM_APPROVED"
	FROM PROGRAM INNER JOIN OPPORTUNITY_REQUEST
		ON PROGRAM.id = OPPORTUNITY_REQUEST.source_program_id
	...
	GROUP BY PROGRAM.id
		UNION
	SELECT PROGRAM.id, "PROGRAM_COMPLETE_APPROVAL_STAGE", 2048, "SYSTEM_ADMINISTRATOR", MIN(PROGRAM_INSTANCE.start_date), "PROGRAM_APPROVED"
	...
	GROUP BY PROGRAM.id
		UNION
	SELECT PROGRAM.id, "PROGRAM_ESCALATE", 2048, "SYSTEM_ADMINISTRATOR", MAX(PROGRAM_INSTANCE.disabled_date), "PROGRAM_DISABLED_PENDING_REACTIVATION"
	FROM PROGRAM INNER JOIN PROGRAM_INSTANCE
		ON PROGRAM.id = PROGRAM_INSTANCE.program_id
	...
	GROUP BY PROGRAM.id
		UNION
	SELECT PROGRAM.id, "PROGRAM_ESCALATE", 2048, "SYSTEM_ADMINISTRATOR", MAX(PROGRAM_INSTANCE.disabled_date), "PROGRAM_DISABLED_PENDING_IMPORT_REACTIVATION"
	FROM PROGRAM INNER JOIN PROGRAM_INSTANCE
		ON PROGRAM.id = PROGRAM_INSTANCE.program_id
	LEFT JOIN OPPORTUNITY_REQUEST
		ON PROGRAM.id = OPPORTUNITY_REQUEST.source_program_id
	...
	GROUP BY PROGRAM.id
		UNION
	SELECT PROGRAM.id, "PROGRAM_ESCALATE", 2048, "SYSTEM_ADMINISTRATOR", DATE(MAX(PROGRAM_INSTANCE.disabled_date) + INTERVAL 2419200 SECOND), "PROGRAM_DISABLED_COMPLETED"
	FROM PROGRAM INNER JOIN PROGRAM_INSTANCE
		ON PROGRAM.id = PROGRAM_INSTANCE.program_id
	WHERE PROGRAM.state_id LIKE "PROGRAM_DISABLED_%"
	GROUP BY PROGRAM.id
