...
public class QualificationInstitutionDAO {
	...
	public List<QualificationInstitution> getAllEnabledInstitutions() {
		return sessionFactory.getCurrentSession().createCriteria(QualificationInstitution.class).add(Restrictions.eq("enabled", true))
	...
	public List<QualificationInstitution> getAllInstitutions() {
		return sessionFactory.getCurrentSession().createCriteria(QualificationInstitution.class).addOrder(Order.asc("name"))
	...
	public List<QualificationInstitution> getAllInstitutionByName(String name) {
		return (List<QualificationInstitution>) sessionFactory.getCurrentSession().createCriteria(QualificationInstitution.class)
				.add(Restrictions.eq("name", name)).addOrder(Order.asc("name")).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
	}
	...
	/**
	 * @deprecated Use
	 *             {@link #getEnableAndValiddInstitutionsByCountryCodeFilteredByNameLikeCaseInsensitive(String, String)}
	 *             instead.
	 */
	@Deprecated
	public List<QualificationInstitution> getEnabledInstitutionsByCountryCodeFilteredByNameLikeCaseInsensitive(String domicileCode, String term) {
		return sessionFactory
				...
				.add(Restrictions.and(
						...
						Restrictions.and(Restrictions.eq("domicileCode", domicileCode),
								Restrictions.or(Restrictions.ilike("name", term, MatchMode.END), Restrictions.ilike("name", term, MatchMode.START)))))
				.addOrder(Order.asc("name")).list();
	}

	/**
	 * Returns all the institutions matching the given domicile code which title
	 * contains the provided term. Additionally, this method filters any
	 * institutions out which are not part of the reference data provided by
	 * PORTICO.
	 * 
	 * @param domicileCode
	 *            the domicile code of the institution
	 * @param term
	 *            a search term such as "Univers" which then finds all
	 *            universities containing "Univers" in its name e.g
	 *            "University of London", "University of Cambridge" etc.
	 * @return a list of matching institutions
	 * 
	 * @deprecated PRISM is switching back to a drop down element rather than
	 *             autosuggest.
	 */
	@Deprecated
	public List<QualificationInstitution> getEnableAndValiddInstitutionsByCountryCodeFilteredByNameLikeCaseInsensitive(String domicileCode, String term) {
		...
		qInstitutionsCriteria.add(
				Restrictions.and(Restrictions.eq("domicileCode", domicileCode),
						Restrictions.or(Restrictions.ilike("name", term, MatchMode.END), Restrictions.ilike("name", term, MatchMode.START)))).addOrder(
				Order.asc("name"));
