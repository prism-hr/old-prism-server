...
public class ProjectDAOTest extends AutomaticRollbackTestCase {
	...
	private void createDomainObjects() {
		RegisteredUser author = new RegisteredUserBuilder().username("author")
				.firstName("author").lastName("project")
				.email("author.project@email.test").build();
		Date closingDate = DateUtils.addDays(new Date(), 10);
		RegisteredUser primarySupervisor = new RegisteredUserBuilder()
				.username("primary").firstName("primary")
				.lastName("supervisor").email("primary.supervisor@email.test")
				.build();
		RegisteredUser secondarySupervisor = new RegisteredUserBuilder()
				.username("secondary").firstName("secondary")
				.lastName("supervisor")
				.email("secondary.supervisor@email.test").build();
		Program program = new ProgramBuilder().code("ProjectProgram").build();
		save(author, primarySupervisor, secondarySupervisor, program);
		Advert advert = new AdvertBuilder().description("desc").funding("fund")
				.studyDuration(1).title("title").build();
		project = new ProjectBuilder().advert(advert).author(author)
				.closingDate(closingDate).primarySupervisor(primarySupervisor)
				.program(program).secondarySupervisor(secondarySupervisor)
				.build();
