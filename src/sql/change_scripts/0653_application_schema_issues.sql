...
/* Add new action definitions for follow up actions */

INSERT INTO ACTION (id)
VALUES ("SYSTEM_VIEW_INSTITUTION_LIST"), 
...
INSERT INTO STATE_ACTION (state_id, action_id, raises_urgent_flag)
	SELECT "SYSTEM_APPROVED", id, 0
	FROM ACTION
	WHERE id LIKE "SYSTEM_VIEW%"
		AND id LIKE "%_LIST"
		OR id IN ("SYSTEM_MANAGE_ACCOUNT", "SYSTEM_ASSIGN_USERS")
		UNION
	SELECT "INSTITUTION_APPROVED", id, 0
	FROM ACTION
	WHERE id LIKE "INSTITUTION_VIEW%"
		AND id LIKE "%_LIST"
		UNION
	SELECT STATE.id, ACTION.id, 0
	FROM STATE INNER JOIN ACTION
	WHERE STATE.id LIKE "PROGRAM%"
		AND ACTION.id LIKE "PROGRAM_VIEW%"
		AND ACTION.id LIKE "%_LIST"
		UNION
	SELECT STATE.id, ACTION.id, 0
	FROM STATE INNER JOIN ACTION
	WHERE STATE.id LIKE "PROJECT%"
		AND ACTION.id LIKE "PROJECT_VIEW%"
		AND ACTION.id LIKE "%_LIST"
;
...
INSERT INTO STATE_ACTION_ASSIGNMENT (state_action_id, role_id)
	SELECT STATE_ACTION.id, ROLE.id
	FROM STATE_ACTION INNER JOIN ROLE
	WHERE STATE_ACTION.action_id LIKE "SYSTEM_VIEW%"
		AND STATE_ACTION.action_id LIKE "%_LIST"
		OR STATE_ACTION.action_id LIKE "SYSTEM_MANAGE_ACCOUNT"
		UNION
	SELECT STATE_ACTION.id, ROLE.id
	FROM STATE_ACTION INNER JOIN ROLE
	WHERE STATE_ACTION.action_id LIKE "INSTITUTION_VIEW%"
		AND STATE_ACTION.action_id LIKE "%_LIST"
		UNION
	SELECT STATE_ACTION.id, ROLE.id
	FROM STATE_ACTION INNER JOIN ROLE
	WHERE STATE_ACTION.action_id LIKE "PROGRAM_VIEW%"
		AND STATE_ACTION.action_id LIKE "%_LIST"
		UNION
	SELECT STATE_ACTION.id, ROLE.id
	FROM STATE_ACTION INNER JOIN ROLE
	WHERE STATE_ACTION.action_id LIKE "PROJECT_VIEW%"
		AND STATE_ACTION.action_id LIKE "%_LIST"
;
