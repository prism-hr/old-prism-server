...
public class ImportedEntityService {
    ...
    public void disableImportedPrograms(Institution institution, List<Integer> updates, LocalDate baseline) {
        ...
        importedEntityDAO.disableImportedAdvertStudyOptions(institution, updates);
    ...
    public Integer mergeImportedProgram(Advert advert, Set<ProgrammeOccurrence> programInstanceDefinitions, LocalDate baseline, DateTime baselineTime)
        ...
        for (ProgrammeOccurrence occurrence : programInstanceDefinitions) {
            ...
            persistentAdvert.getStudyOptions().add(persistentAdvertStudyOption);
    ...
    private Program mergeProgram(Institution institution, Programme programDefinition, LocalDate baseline) throws DeduplicationException {
        ...
        PrismAdvertType advertTypeId = PrismAdvertType.findValueFromString(programDefinition.getName());
        advertTypeId = advertTypeId == null ? getSystemAdvertType() : advertTypeId;
        ...
        Program transientProgram = new Program().withSystem(systemService.getSystem()).withInstitution(institution).withDepartment(department)
                ...
                .withImported(true).withUser(proxyCreator).withCreatedTimestamp(baselineDateTime)
        ...
        if (persistentProgram == null) {
            Advert institutionAdvert = institution.getAdvert();
            AdvertType advertType = getImportedEntityByCode(AdvertType.class, institution, advertTypeId.name());
            Advert transientAdvert = new Advert().withTitle(transientTitle).withLocale(institutionAdvert.getLocale())
                    .withCurrency(institutionAdvert.getCurrency()).withAdvertType(advertType)
                    .withAddress(advertService.createAddressCopy(institutionAdvert.getAddress()));
