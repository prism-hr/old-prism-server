...
ALTER TABLE ADVERT
	ADD COLUMN immediate_start INT(1) UNSIGNED AFTER description
;

UPDATE ADVERT INNER JOIN PROJECT
ON ADVERT.id = PROJECT.id
SET immediate_start = 1
;

UPDATE ADVERT INNER JOIN PROGRAM
ON ADVERT.id = PROGRAM.id
SET immediate_start = 0
;

ALTER TABLE ADVERT
	MODIFY COLUMN immediate_start INT(1) UNSIGNED NOT NULL
;
