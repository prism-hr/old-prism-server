...
public class ApplicationFormUserRoleDAO {
    ...
    public Boolean findRaisesUpdateFlagByUserAndApplicationForm(RegisteredUser user, ApplicationForm applicationForm) {
        Boolean raisesUpdateFlag = (Boolean) sessionFactory.getCurrentSession().createCriteria(ApplicationFormUserRole.class)
                .add(Restrictions.eq("applicationForm", applicationForm)).add(Restrictions.eq("user", user)).addOrder(Order.desc("raisesUpdateFlag"))
                .setProjection(Projections.projectionList().add(Projections.max("raisesUpdateFlag"))).uniqueResult();
    ...
    public Boolean findRaisesUrgentFlagByUserAndApplicationForm(RegisteredUser user, ApplicationForm applicationForm) {
        Boolean raisesUrgentFlag = (Boolean) sessionFactory.getCurrentSession().createCriteria(ApplicationFormUserRole.class)
                .add(Restrictions.eq("applicationForm", applicationForm)).add(Restrictions.eq("user", user)).addOrder(Order.desc("raisesUrgentFlag"))
    ...
    public boolean checkActionAvailableForUserAndApplicationForm(RegisteredUser registeredUser, ApplicationForm applicationForm, ApplicationFormAction action) {
        ...
        for (ActionDefinition availableAction : availableActions) {
            if (availableAction.getAction() == action
                    || (availableAction.getAction() == ApplicationFormAction.VIEW_EDIT && action == ApplicationFormAction.VIEW)) {
                return true;
            }
        }
        return false;
    ...
    public List<RegisteredUser> findUsersInterestedInApplication(ApplicationForm applicationForm) {
        return sessionFactory
                .getCurrentSession()
                .createCriteria(ApplicationFormUserRole.class)
    ...
    public List<RegisteredUser> findUsersPotentiallyInterestedInApplication(ApplicationForm applicationForm) {
        ...
        return sessionFactory.getCurrentSession().createCriteria(ApplicationFormUserRole.class)
                ...
                .createAlias("user", "registeredUser", JoinType.INNER_JOIN).add(Restrictions.eq("program.id", applicationForm.getAdvert().getId()))
    ...
    public void deleteApplicationActions(ApplicationForm applicationForm) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_DELETE_APPLICATION_ACTIONS(?);").setInteger(0, applicationForm.getId());
        query.executeUpdate();
    }

    public void deleteApplicationUpdate(ApplicationForm applicationForm, RegisteredUser registeredUser) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_DELETE_APPLICATION_UPDATE(?, ?);").setInteger(0, applicationForm.getId())
    ...
    public void deleteProgramRole(RegisteredUser registeredUser, Program program, Authority authority) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_DELETE_PROGRAM_ROLE(?, ?, ?);").setInteger(0, registeredUser.getId())
                .setInteger(1, program.getId()).setString(2, authority.toString());
        query.executeUpdate();
    }

    public void deleteStateActions(ApplicationForm applicationForm) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_DELETE_STATE_ACTIONS(?);").setInteger(0, applicationForm.getId());
        query.executeUpdate();
    }

    public void deleteRoleAction(ApplicationForm applicationForm, Authority authority, ApplicationFormAction action) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_DELETE_ROLE_ACTION(?, ?, ?);").setInteger(0, applicationForm.getId())
                .setString(1, authority.toString()).setString(2, action.toString());
        query.executeUpdate();
    }
    ...
    public void deleteUserAction(ApplicationForm applicationForm, RegisteredUser registeredUser, Authority authority, ApplicationFormAction action) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_DELETE_USER_ACTION(?, ?, ?, ?);").setInteger(0, applicationForm.getId())
                .setInteger(1, registeredUser.getId()).setString(2, authority.toString()).setString(3, action.toString());
        query.executeUpdate();
    }

    public void deleteUserRole(RegisteredUser registeredUser, Authority authority) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_DELETE_USER_ROLE(?, ?);").setInteger(0, registeredUser.getId())
    ...
    public void insertApplicationUpdate(ApplicationForm applicationForm, RegisteredUser registeredUser, Date updateTimestamp,
        ...
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_INSERT_APPLICATION_UPDATE(?, ?, ?, ?);")
                .setInteger(0, applicationForm.getId()).setInteger(1, registeredUser.getId()).setString(2, javaDateToMySQLDateString(updateTimestamp))
    ...
    public void insertProgramRole(RegisteredUser registeredUser, Program program, Authority authority) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_INSERT_PROGRAM_ROLE(?, ?, ?);").setInteger(0, registeredUser.getId())
                .setInteger(1, program.getId()).setString(2, authority.toString());
        query.executeUpdate();
    }

    public void insertUserRole(RegisteredUser registeredUser, Authority authority) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_INSERT_USER_ROLE(?, ?);").setInteger(0, registeredUser.getId())
                .setString(1, authority.toString());
        query.executeUpdate();
    }
    ...
    public List<ActionDefinition> selectUserActions(Integer registeredUserId, Integer applicationFormId, ApplicationFormStatus status) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_SELECT_USER_ACTIONS(?, ?, ?);").setInteger(0, registeredUserId)
                .setInteger(1, applicationFormId).setString(2, status.toString());

        List<Object[]> rows = (List<Object[]>) query.list();
        List<ActionDefinition> actionDefinitions = new ArrayList<ActionDefinition>();

        Boolean hasCorrectApplicationFormAction = false;
        ...
        for (Object[] row : rows) {
            ...
            if (action == ApplicationFormAction.CORRECT_APPLICATION) {
                hasCorrectApplicationFormAction = true;
            }
            ...
            if (action == ApplicationFormAction.VIEW_EDIT && BooleanUtils.isTrue(hasViewAction)) {
                actionDefinitions.remove(actionDefinitions.size() - 1);
            }

            if (!(Arrays.asList(ApplicationFormAction.VIEW, ApplicationFormAction.VIEW_EDIT).contains(action) && BooleanUtils
                    .isTrue(hasCorrectApplicationFormAction)))
    ...
    public void updateApplicationDueDate(ApplicationForm applicationForm, Date deadlineTimestamp) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_UPDATE_APPLICATION_FORM_DUE_DATE(?, ?);")
                .setInteger(0, applicationForm.getId()).setDate(1, deadlineTimestamp);
        query.executeUpdate();
    }

    public void updateApplicationInterest(ApplicationForm applicationForm, RegisteredUser registeredUser, Boolean interested) {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_UPDATE_APPLICATION_INTEREST(?, ?, ?);").setInteger(0, applicationForm.getId())
                .setInteger(1, registeredUser.getId()).setBoolean(2, interested);
        query.executeUpdate();
    }

    public void updateUrgentApplications() {
        Query query = sessionFactory.getCurrentSession().createSQLQuery("CALL SP_UPDATE_URGENT_APPLICATIONS();");
        query.executeUpdate();
    }
