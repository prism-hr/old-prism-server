...
public class ExportQueueListener implements MessageListener {
    ...
    private void sendEmailWithErrorMessage(final ApplicationForm application) {
        try {
        	...
        	mailSendingService.sendExportErrorMessage(userService.getUsersInRole(Authority.SUPERADMINISTRATOR, Authority.ADMITTER), messageCode, new Date(), application);
        } catch (Exception ex) {
            log.warn("{}", ex);
        }
    }
    ...
    private void disablePorticoInterface() {
        ...
        try {
            mailSendingService.sendExportErrorMessage(userService.getUsersInRole(Authority.SUPERADMINISTRATOR), messageCode, new Date(), null);
        } catch (Exception ex) {
            log.warn("{}", ex);
        }
    }
