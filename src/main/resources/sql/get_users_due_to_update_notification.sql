...
FROM (
	...
	GROUP BY APPLICATION_FORM_USER_ROLE.registered_user_id) AS USER_NOTIFICATION_LIST
WHERE DATE(USER_NOTIFICATION_LIST.updated_on_timestamp) = CURRENT_DATE() 
	AND USER_NOTIFICATION_LIST.has_update_flag = 1
	AND USER_NOTIFICATION_LIST.has_urgent_flag IS NULL
	OR USER_NOTIFICATION_LIST.has_urgent_flag = 0
;