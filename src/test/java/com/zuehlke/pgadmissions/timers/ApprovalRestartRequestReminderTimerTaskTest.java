...
public class ApprovalRestartRequestReminderTimerTaskTest {
		private  ApprovalRestartRequestReminderTimerTask notificationTask;
		...
		public void setup() {
			...
			notificationTask = new ApprovalRestartRequestReminderTimerTask(sessionFactoryMock, mailSenderMock, applicationServiceMock);
		}

		@Test
		public void shouldSendApprovalRestartRequestReminders() throws Exception {
			...
			EasyMock.expect(applicationServiceMock.getApplicationsDueApprovalRestartRequestReminder()).andReturn(applicationFormList);
			transactionOne.commit();

			mailSenderMock.sendRequestRestartApprovalReminder(applicationFormOne);
			applicationServiceMock.save(applicationFormOne);
			transactionTwo.commit();

			mailSenderMock.sendRequestRestartApprovalReminder(applicationFormTwo);
			...
			assertEquals(DateUtils.truncate(new Date(), Calendar.DATE), DateUtils.truncate(applicationFormOne.getNotificationForType(NotificationType.APPROVAL_RESTART_REQUEST_REMINDER).getDate(), Calendar.DATE));
			assertEquals(DateUtils.truncate(new Date(), Calendar.DATE), DateUtils.truncate(applicationFormTwo.getNotificationForType(NotificationType.APPROVAL_RESTART_REQUEST_REMINDER).getDate(), Calendar.DATE));
		...
		public void shouldRollBackTransactionIfExceptionOccurs() throws Exception {
			...
			assertNull(applicationFormOne.getNotificationForType(NotificationType.APPROVAL_RESTART_REQUEST_REMINDER));
			assertEquals(DateUtils.truncate(new Date(), Calendar.DATE), DateUtils.truncate(applicationFormTwo.getNotificationForType(NotificationType.APPROVAL_RESTART_REQUEST_REMINDER).getDate(), Calendar.DATE));
		}
	}
