...
public class ImportedEntityService {
    ...
    public Integer mergeImportedProgram(Institution institution, Set<ProgrammeOccurrence> programInstanceDefinitions, LocalDate baseline, DateTime baselineTime)
        ...
        for (ProgrammeOccurrence occurrence : programInstanceDefinitions) {
            ...
            AdvertStudyOption transientAdvertStudyOption = new AdvertStudyOption().withAdvert(persistentAdvert).withStudyOption(studyOption)
            ...
            AdvertStudyOption persistentAdvertStudyOption = mergeAdvertStudyOption(transientAdvertStudyOption, baseline);
            persistentAdvert.getAdvertStudyOptions().add(persistentAdvertStudyOption);

            AdvertStudyOptionInstance transientAdvertStudyOptionInstance = new AdvertStudyOptionInstance().withStudyOption(persistentAdvertStudyOption)
                    .withApplicationStartDate(transientStartDate).withApplicationCloseDate(transientCloseDate)
                    .withAcademicYear(Integer.toString(transientStartDate.getYear())).withIdentifier(occurrence.getIdentifier())
                    .withEnabled(transientCloseDate.isAfter(baseline));

            AdvertStudyOptionInstance persistentAdvertStudyOptionInstance = entityService.createOrUpdate(transientAdvertStudyOptionInstance);
            persistentAdvertStudyOption.getStudyOptionInstances().add(persistentAdvertStudyOptionInstance);
    ...
    private Program mergeProgram(Institution institution, Programme programDefinition, LocalDate baseline) throws DeduplicationException {
        ...
        PrismProgramType prismProgramType = PrismProgramType.findValueFromString(programDefinition.getName());
        prismProgramType = prismProgramType == null ? institution.getProgramType() : prismProgramType;

        boolean transientRequireProjectDefinition = programDefinition.isAtasRegistered();

        DateTime baselineDateTime = new DateTime();
        ProgramType programType = getImportedEntityByCode(ProgramType.class, institution, prismProgramType.name());
        ...
        Program transientProgram = new Program().withSystem(systemService.getSystem()).withInstitution(institution).withDepartment(department)
                .withImportedCode(programDefinition.getCode()).withTitle(transientTitleClean).withLocale(institution.getLocale())
                .withProgramType(prismProgramType).withRequireProjectDefinition(transientRequireProjectDefinition).withImported(true)
                .withImportedProgramType(programType).withUser(proxyCreator).withCreatedTimestamp(baselineDateTime).withUpdatedTimestamp(baselineDateTime)
    ...
    private AdvertStudyOption mergeAdvertStudyOption(AdvertStudyOption transientAdvertStudyOption, LocalDate baseline) throws DeduplicationException {
        AdvertStudyOption persistentAdvertStudyOption = entityService.getDuplicateEntity(transientAdvertStudyOption);

        if (persistentAdvertStudyOption == null) {
            entityService.save(transientAdvertStudyOption);
            return transientAdvertStudyOption;
        } else {
            LocalDate transientStartDate = transientAdvertStudyOption.getApplicationStartDate();
            LocalDate transientCloseDate = transientAdvertStudyOption.getApplicationCloseDate();

            LocalDate persistentStartDate = persistentAdvertStudyOption.getApplicationStartDate();
            LocalDate persistentCloseDate = persistentAdvertStudyOption.getApplicationCloseDate();
            ...
            persistentAdvertStudyOption.setApplicationStartDate(persistentStartDate);
            persistentAdvertStudyOption.setApplicationCloseDate(persistentCloseDate);

            persistentAdvertStudyOption.setEnabled(persistentCloseDate.isAfter(baseline));
            return persistentAdvertStudyOption;
        }
    }
    ...
    private StudyOption mergeStudyOption(Institution institution, ModeOfAttendance modeOfAttendance) throws DeduplicationException {
        ...
        studyOptionId = studyOptionId == null ? FULL_TIME : studyOptionId;
