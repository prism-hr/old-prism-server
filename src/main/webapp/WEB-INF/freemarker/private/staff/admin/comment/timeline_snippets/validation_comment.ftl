<h3 class="answer <#if comment.qualifiedForPhd??>${comment.qualifiedForPhd?string?lower_case}</#if>">
	<span data-desc="${comment.qualifiedForPhd?string!"Unsure"}"></span> Is the applicant qualified for PhD entry to UCL?
</h3>
...
<h3 class="answer <#if comment.homeOrOverseas??>${comment.homeOrOverseas?string?lower_case}</#if>">
	<span data-desc="${comment.homeOrOverseas?string!"Unsure"}"></span> What is the applicant's fee status?
</h3>
