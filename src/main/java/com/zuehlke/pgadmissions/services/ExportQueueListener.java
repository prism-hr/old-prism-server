...
public class ExportQueueListener implements MessageListener {
    ...
    private void sendEmailWithErrorMessage(final ApplicationForm application) {
        try {
        	...
        	mailSendingService.sendExportErrorMessage(roleService.getUsersInRole(roleService.getPrismSystem(), Authority.SUPERADMINISTRATOR, Authority.ADMITTER), messageCode, new Date(), application);
        } catch (Exception ex) {
            log.warn("{}", ex);
        }
    }
    ...
    private void disablePorticoInterface() {
        ...
        try {
            mailSendingService.sendExportErrorMessage(roleService.getUsersInRole(roleService.getPrismSystem(), Authority.SUPERADMINISTRATOR), messageCode, new Date(), null);
        } catch (Exception ex) {
            log.warn("{}", ex);
        }
    }
