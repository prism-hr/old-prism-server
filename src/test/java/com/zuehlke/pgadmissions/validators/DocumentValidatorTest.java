...
public class DocumentValidatorTest {
    ...
    public void shouldNotAllowDamagedPdf() throws IOException {
        ...
        documentValidator.validate(document, mappingResult);
        Assert.assertEquals(1, mappingResult.getErrorCount());
        Assert.assertEquals("file.upload.corrupted", mappingResult.getFieldError("content").getCode());
    }

    @Test
    public void shouldNotAllowRestrictedPdf() throws IOException {
        DirectFieldBindingResult mappingResult = new DirectFieldBindingResult(document, "document");
        document.setFileName("copy_restricted.pdf");
        document.setContent(FileUtils.readFileToByteArray(new File("src/test/resources/pdf/copy_restricted.pdf")));
        documentValidator.validate(document, mappingResult);
        Assert.assertEquals(1, mappingResult.getErrorCount());
        Assert.assertEquals("file.upload.encrypted", mappingResult.getFieldError("content").getCode());
