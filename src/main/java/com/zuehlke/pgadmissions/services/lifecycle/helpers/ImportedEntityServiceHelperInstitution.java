...
public class ImportedEntityServiceHelperInstitution implements AbstractServiceHelper {
    ...
    public void execute() throws Exception {
        ...
        for (ImportedEntityFeed importedEntityFeed : importedEntityService.getImportedEntityFeeds()) {
            ...
            try {
                ...
                notificationService.sendDataImportErrorNotifications(importedEntityFeed.getAdvert().getResourceParent().getInstitution(), errorMessage);
            } finally {
    ...
    private void importEntities(ImportedEntityFeed importedEntityFeed) throws Exception {
        Advert advert = importedEntityFeed.getAdvert();
        if (contextEnvironment.equals("prod") || !advertService.hasAuthenticatedFeeds(advert)) {
            ...
            if (unmarshalled != null) {
                ...
                if (importedEntityClass.equals(Program.class)) {
                    mergeImportedPrograms(importedEntityFeedId, advert, (List<ProgrammeOccurrence>) unmarshalled);
                } else if (importedEntityClass.equals(ImportedInstitution.class)) {
                    mergeImportedInstitutions(importedEntityFeedId, advert,
                    ...
                    mergeImportedLanguageQualificationTypes(importedEntityFeedId, advert,
                            (List<com.zuehlke.pgadmissions.referencedata.jaxb.LanguageQualificationTypes.LanguageQualificationType>) unmarshalled);
                } else {
                    mergeImportedEntities(importedEntityFeedId, advert, (Class<ImportedEntity>) importedEntityClass,
    ...
    private void mergeImportedPrograms(Integer importedEntityFeedId, Advert advert, List<ProgrammeOccurrence> programDefinitions) throws Exception {
        ...
        for (String programCode : batchedOccurrences.keySet()) {
            ...
            updates.add(importedEntityService.mergeImportedProgram(advert, occurrencesInBatch, baseline, baselineTime));
        }

        importedEntityService.disableImportedPrograms(advert, updates, baseline);
