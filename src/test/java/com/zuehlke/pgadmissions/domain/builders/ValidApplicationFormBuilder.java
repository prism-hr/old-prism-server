...
public class ValidApplicationFormBuilder {
    ...
    public ApplicationForm build() {
        ...
        user = new RegisteredUserBuilder().id(Integer.MAX_VALUE).firstName("Kevin").firstName2("Franciszek").firstName3("Duncan").lastName("Denver").username("denk@zhaw.ch").email("ked@zuhlke.com").enabled(true).build();
        ...
        referenceComment1 = new ReferenceCommentBuilder().comment("Hello From Bob").referee(refereeOne).document(referenceDocument).providedBy(user).suitableForProgramme(true).suitableForUcl(true).user(user).build();
        referenceComment2 = new ReferenceCommentBuilder().comment("Hello From Jane").referee(refereeTwo).document(referenceDocument).providedBy(user).suitableForProgramme(true).suitableForUcl(true).user(user).build();
        refereeOne = new RefereeBuilder().id(Integer.MAX_VALUE-1).user(approverUser).email("ked1@zuhlke.com").firstname("Bob").lastname("Smith").addressCountry(country).address1(addressStr.split("\n")[0]).address2(addressStr.split("\n")[1]).address3(addressStr.split("\n")[2]).address4(addressStr.split("\n")[3]).address5(addressStr.split("\n")[4]).jobEmployer("Zuhlke Engineering Ltd.").jobTitle("Software Engineer").messenger("skypeAddress").phoneNumber("+44 (0) 123 123 1234").sendToUCL(true).reference(referenceComment1).build();
        refereeTwo = new RefereeBuilder().id(Integer.MAX_VALUE-2).user(approverUser).email("ked2@zuhlke.com").firstname("Jane").lastname("Austen").addressCountry(country).address1(addressStr.split("\n")[0]).address2(addressStr.split("\n")[1]).address3(addressStr.split("\n")[2]).address4(addressStr.split("\n")[3]).address5(addressStr.split("\n")[4]).jobEmployer("Zuhlke Engineering Ltd.").jobTitle("Software Engineer").messenger("skypeAddress").phoneNumber("+44 (0) 123 123 1234").sendToUCL(true).reference(referenceComment2).build();
