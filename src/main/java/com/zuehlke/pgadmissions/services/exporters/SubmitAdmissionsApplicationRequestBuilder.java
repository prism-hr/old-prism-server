...
public class SubmitAdmissionsApplicationRequestBuilder {
	...
	private static class NoActiveProgrameInstanceFoundException extends
			RuntimeException {
		private final ProgrammeOccurrenceTp occurrenceTp;
		private static final long serialVersionUID = 8359986556018188704L;

		public NoActiveProgrameInstanceFoundException(
				ProgrammeOccurrenceTp occurrenceTp, String message) {
			super(message);
	...
	private static class NoIdentifierForProgrameInstanceFoundException extends
			RuntimeException {
		private static final long serialVersionUID = 1820912139538020762L;
		private final ProgrammeOccurrenceTp occurrenceTp;

		public NoIdentifierForProgrameInstanceFoundException(
				ProgrammeOccurrenceTp occurrenceTp, String message) {
			super(message);
			this.occurrenceTp = occurrenceTp;
		}
	...
	public SubmitAdmissionsApplicationRequestBuilder applicationForm(
			final ApplicationForm applicationForm) {
		this.applicationForm = applicationForm;
		return this;
	}

	public SubmitAdmissionsApplicationRequestBuilder isOverseasStudent(
			Boolean isOverseasStudent) {
	...
	public SubmitAdmissionsApplicationRequest build() {
		...
		SubmitAdmissionsApplicationRequest request = xmlFactory
				.createSubmitAdmissionsApplicationRequest();
	...
	private ApplicantTp buildApplicant() {
		...
		applicant.setVisaRequired(BooleanUtils.toBoolean(applicationForm
				.getPersonalDetails().getRequiresVisa()));
		if (BooleanUtils.isTrue(applicationForm.getPersonalDetails()
				.getRequiresVisa())) {
		...
		applicant.setCriminalConvictionDetails(cleanString(applicationForm
				.getAdditionalInformation().getConvictionsText()));
		applicant.setCriminalConvictions(applicationForm
				.getAdditionalInformation().getConvictions());
		applicant.setQualificationList(buildQualificationDetails());
		applicant.setEmployerList(buildEmployer());

		applicant.setEnglishIsFirstLanguage(BooleanUtils
				.toBoolean(applicationForm.getPersonalDetails()
						.getEnglishFirstLanguage()));
		applicant
				.setEnglishLanguageQualificationList(buildEnglishLanguageQualification());

		if (BooleanUtils.isNotTrue(applicationForm.getPersonalDetails()
				.getEnglishFirstLanguage())
				&& BooleanUtils.isNotTrue(applicationForm.getPersonalDetails()
						.getLanguageQualificationAvailable())) {
			printLanguageQualificationAdmissionsNote = true;
		}

		applicant.setApplicantID(StringUtils.trimToNull(applicationForm
				.getApplicant().getUclUserId()));
	...
	private DomicileTp buildDomicile() {
		...
		domicileTp.setCode(applicationForm.getPersonalDetails()
				.getResidenceCountry().getCode());
		domicileTp.setName(applicationForm.getPersonalDetails()
				.getResidenceCountry().getName());
		return domicileTp;
	}
	...
	private GenderTp buildSex() {
		...
		if (gender == Gender.MALE) {
			...
			throw new IllegalArgumentException(String.format(
					"Gender type [%s] could not be converted", gender));
		}
	}
	...
	private NationalityTp buildFirstNationality() {
		...
		if (firstNationality == null) {
			throw new IllegalArgumentException(
					"Candidate should have at least one nationality.");
		}
	...
	private NationalityTp buildSecondNationality() {
		Language secondNationality = applicationForm.getPersonalDetails()
	...
	private PassportTp buildPassport() {
		PersonalDetails personalDetails = applicationForm.getPersonalDetails();
		PassportInformation passportInformation = personalDetails
		...
		if (passportInformation != null) {
			...
			passportTp.setExpiryDate(buildXmlDate(passportInformation
					.getPassportExpiryDate()));
			passportTp.setIssueDate(buildXmlDate(passportInformation
			...
			passportTp.setExpiryDate(buildXmlDate(DateUtils.addYears(
					new Date(), 1)));
			passportTp.setIssueDate(buildXmlDate(DateUtils.addYears(new Date(),
					-1)));
			return passportTp;
		}
	}
	...
	private DisabilityTp buildDisability() {
		...
		disabilityTp.setCode(Integer.toString(personalDetails.getDisability()
				.getCode()));
	...
	private EthnicityTp buildEthnicity() {
		...
		ethnicityTp.setCode(Integer.toString(personalDetails.getEthnicity()
				.getCode()));
	...
	private ContactDtlsTp buildHomeAddress() {
		...
		contactDtlsTp.setLandline(cleanPhoneNumber(personalDetails
				.getPhoneNumber()));
	...
	private ContactDtlsTp buildCorrespondenceAddress() {
		...
		contactDtlsTp.setAddressDtls(addressTp);
		contactDtlsTp.setEmail(applicationForm.getApplicant().getEmail());
		contactDtlsTp.setLandline(cleanPhoneNumber(applicationForm
				.getPersonalDetails().getPhoneNumber()));
		return contactDtlsTp;
	}

	private CourseApplicationTp buildCourseApplication() {
		ProgrammeDetails programmeDetails = applicationForm
				.getProgrammeDetails();
		CourseApplicationTp applicationTp = xmlFactory
				.createCourseApplicationTp();
		applicationTp.setStartMonth(new DateTime(programmeDetails
				.getStartDate()));

		NameTp supervisorName = buildAgreedSupervisorName();
		if (supervisorName == null
				&& !programmeDetails.getSuggestedSupervisors().isEmpty()) {
		...
		applicationTp
				.setSourcesOfInterest(buildSourcesOfInterest(applicationTp));
		applicationTp.setCreationDate(buildXmlDate(applicationForm
				.getSubmittedDate()));
		applicationTp.setIpAddress(applicationForm.getIpAddressAsString());
		applicationTp.setExternalApplicationID(applicationForm.getApplication()
		...
		case REJECTED:
			...
			break;
		default:
			throw new IllegalArgumentException("Application is in wrong state "
					+ applicationForm.getStatus().displayValue());
		}
		...
		if (printLanguageQualificationAdmissionsNote
			...
			applicationTp
					.setDepartmentalOfferConditions(LANGUAGE_QUALIFICATION_ADMISSIONS_NOTE);
		}

		ApprovalRound latestApprovalRound = applicationForm
				.getLatestApprovalRound();

		if (latestApprovalRound != null) {
			if (isOverseasStudent
					&& BooleanUtils.isTrue(applicationForm.getProgram()
							.getAtasRequired())) {
				applicationTp.setAtasStatement(latestApprovalRound
						.getProjectAbstract());
			}

			if (applicationForm.getStatus() == ApplicationFormStatus.APPROVED) {
				if (BooleanUtils.isTrue(latestApprovalRound
						.getRecommendedConditionsAvailable())) {
					applicationTp
							.setDepartmentalOfferConditions("Conditional Offer: "
									+ latestApprovalRound
											.getRecommendedConditions());
				} else {
					applicationTp
							.setDepartmentalOfferConditions("Unconditional Offer");
				}
			}
		}
	...
	private ProgrammeOccurrenceTp buildProgrammeOccurence() {
		...
		ProgrammeDetails programmeDetails = applicationForm
				.getProgrammeDetails();
		ProgrammeOccurrenceTp occurrenceTp = xmlFactory
				.createProgrammeOccurrenceTp();
		...
		for (ProgramInstance instance : program.getInstances()) {
			if (com.zuehlke.pgadmissions.utils.DateUtils.isToday(instance
					.getApplicationStartDate())
					|| instance.getApplicationStartDate().after(new Date())) {
				if (instance.getStudyOption().equalsIgnoreCase(
		...
		if (activeInstance == null) {
			occurrenceTp.setAcademicYear(buildXmlDateYearOnly(programmeDetails
					.getStartDate()));
			occurrenceTp.setIdentifier(NOT_PROVIDED_VALUE);
			occurrenceTp.setStartDate(buildXmlDate(programmeDetails
					.getStartDate()));
			occurrenceTp.setEndDate(buildXmlDate(DateUtils.addYears(
					programmeDetails.getStartDate(), 1)));
			throw new NoActiveProgrameInstanceFoundException(
					occurrenceTp,
					String.format(
		...
		occurrenceTp.setAcademicYear(buildXmlDateYearOnly(activeInstance
				.getAcademic_year()));
		occurrenceTp.setIdentifier(activeInstance.getIdentifier());
		occurrenceTp.setStartDate(buildXmlDate(activeInstance
				.getApplicationStartDate()));
		occurrenceTp.setEndDate(buildXmlDate(activeInstance
				.getApplicationDeadline()));

		if (StringUtils.isBlank(activeInstance.getIdentifier())) {
			occurrenceTp.setIdentifier(NOT_PROVIDED_VALUE);
			throw new NoIdentifierForProgrameInstanceFoundException(
					occurrenceTp,
					String.format(
							"No identifier for program instance found. Program[code=%s]",
							program.getCode()));
		}
	...
	private ModeofattendanceTp buildModeofattendance() {
		ProgrammeDetails programmeDetails = applicationForm
				.getProgrammeDetails();
		ModeofattendanceTp modeofattendanceTp = xmlFactory
				.createModeofattendanceTp();
	...
	private NameTp buildSuggestedSupervisorName(int idx) {
		ProgrammeDetails programmeDetails = applicationForm
				.getProgrammeDetails();
		NameTp nameTp = xmlFactory.createNameTp();
		List<SuggestedSupervisor> suggestedSupervisors = programmeDetails
				.getSuggestedSupervisors();

		if (idx < suggestedSupervisors.size()) {
			SuggestedSupervisor suggestedSupervisor = suggestedSupervisors
					.get(idx);
	...
	private SourceOfInterestTp buildSourcesOfInterest(
			CourseApplicationTp applicationTp) {
		ProgrammeDetails programmeDetails = applicationForm
				.getProgrammeDetails();
		SourceOfInterestTp interestTp = xmlFactory.createSourceOfInterestTp();
		SourcesOfInterest sourcesOfInterest = programmeDetails
				.getSourcesOfInterest();
		interestTp.setCode(sourcesOfInterest.getCode());
		interestTp.setName(sourcesOfInterest.getName());
		if (sourcesOfInterest.isFreeText()) {
			applicationTp.setOtherSourceofInterest(programmeDetails
					.getSourcesOfInterestText());
		}
		return interestTp;
	}

	private QualificationDetailsTp buildQualificationDetails() {
		QualificationDetailsTp resultList = xmlFactory
				.createQualificationDetailsTp();

		List<Qualification> qualifications = applicationForm
				.getQualifications();
		if (!qualifications.isEmpty()) {
			for (Qualification qualification : qualifications) {
				QualificationsTp qualificationsTp = xmlFactory
						.createQualificationsTp();

				qualificationsTp.setStartDate(buildXmlDate(qualification
				...
				qualificationsTp.setEndDate(buildXmlDate(qualification
						.getQualificationAwardDate()));

				qualificationsTp
						.setGrade(qualification.getQualificationGrade());
				qualificationsTp.setLanguageOfInstruction(qualification
						.getQualificationLanguage());
				qualificationsTp.setMainSubject(qualification
						.getQualificationSubject());

				QualificationTp qualificationTp = xmlFactory
						.createQualificationTp();
				qualificationTp.setCode(qualification.getQualificationType()
						.getCode());
				qualificationTp.setName(qualification.getQualificationType()
						.getName());
				qualificationsTp.setQualification(qualificationTp);

				InstitutionTp institutionTp = xmlFactory.createInstitutionTp();

				institutionTp.setCode(qualification
						.getQualificationInstitutionCode());
				institutionTp.setName(qualification
						.getQualificationInstitution());

				CountryTp countryTp = xmlFactory.createCountryTp();
				countryTp.setCode(qualification.getInstitutionCountry()
						.getCode());
				countryTp.setName(qualification.getInstitutionCountry()
						.getName());
			...
			QualificationsTp qualificationsTp = xmlFactory
					.createQualificationsTp();
			qualificationsTp.setStartDate(buildXmlDate(DateUtils.addYears(
					new Date(), -1)));
			qualificationsTp.setEndDate(buildXmlDate(DateUtils.addYears(
					new Date(), 1)));
	...
	private EmploymentDetailsTp buildEmployer() {
		...
		List<EmploymentPosition> employmentPositions = applicationForm
		...
		if (!employmentPositions.isEmpty()) {
			for (EmploymentPosition employmentPosition : employmentPositions) {
				...
				appointmentTp
						.setResponsibilities(cleanString(employmentPosition
								.getRemit()));
				appointmentTp.setStartDate(buildXmlDate(employmentPosition
						.getStartDate()));
				appointmentTp.setEndDate(buildXmlDate(employmentPosition
						.getEndDate()));
	...
	private RefereeListTp buildReferee() {
		...
		for (Referee referee : applicationForm.getRefereesToSendToPortico()) {
			...
			contactDtlsTp
					.setLandline(cleanPhoneNumber(referee.getPhoneNumber()));

			if (StringUtils.isBlank(referee.getPhoneNumber())) {
				contactDtlsTp.setLandline(PHONE_NUMBER_NOT_PROVIDED_VALUE);
			} else if (!ESAPI.validator().isValidInput("PhoneNumber",
					referee.getPhoneNumber(), "PhoneNumber", 25, false)) {
				contactDtlsTp.setLandline(PHONE_NUMBER_NOT_PROVIDED_VALUE);
			}
			...
			addressTp.setAddressLine1(referee.getAddressLocation()
					.getAddress1());
			addressTp.setAddressLine2(referee.getAddressLocation()
					.getAddress2());
			addressTp.setAddressLine3(referee.getAddressLocation()
					.getAddress3());
			addressTp.setAddressLine4(referee.getAddressLocation()
					.getAddress4());
			addressTp.setPostCode(referee.getAddressLocation().getAddress5());
			addressTp.setCountry(referee.getAddressLocation().getDomicile()
					.getCode());
	...
	private EnglishLanguageQualificationDetailsTp buildEnglishLanguageQualification() {
		...
		for (LanguageQualification languageQualifications : personalDetails
				.getLanguageQualifications()) {
			EnglishLanguageTp englishLanguageTp = xmlFactory
					.createEnglishLanguageTp();
			englishLanguageTp.setDateTaken(buildXmlDate(languageQualifications
					.getDateOfExamination()));

			if (languageQualifications.getQualificationType() == LanguageQualificationEnum.OTHER) {
				englishLanguageTp
						.setLanguageExam(QualificationsinEnglishTp.OTHER);
				englishLanguageTp.setOtherLanguageExam(languageQualifications
						.getOtherQualificationTypeName());
			} else if (languageQualifications.getQualificationType() == LanguageQualificationEnum.TOEFL) {
				englishLanguageTp
						.setLanguageExam(QualificationsinEnglishTp.TOEFL);
				...
				englishLanguageTp
						.setLanguageExam(QualificationsinEnglishTp.IELTS);
			} else {
				throw new IllegalArgumentException(String.format(
						"QualificationType type [%s] could not be converted",
			...
			EnglishLanguageScoreTp overallScoreTp = xmlFactory
					.createEnglishLanguageScoreTp();
			overallScoreTp.setName(LanguageBandScoreTp.OVERALL);
			overallScoreTp.setScore(languageQualifications.getOverallScore()
					.replace(".0", ""));

			EnglishLanguageScoreTp readingScoreTp = xmlFactory
					.createEnglishLanguageScoreTp();
			readingScoreTp.setName(LanguageBandScoreTp.READING);
			readingScoreTp.setScore(languageQualifications.getReadingScore()
					.replace(".0", ""));

			EnglishLanguageScoreTp writingScoreTp = xmlFactory
					.createEnglishLanguageScoreTp();
			writingScoreTp.setName(LanguageBandScoreTp.WRITING);
			writingScoreTp.setScore(languageQualifications.getWritingScore()
					.replace(".0", ""));

			EnglishLanguageScoreTp essayOrSpeakingScoreTp = null;
			if (StringUtils.equalsIgnoreCase("TOEFL_PAPER",
					englishLanguageTp.getMethod())) {
				essayOrSpeakingScoreTp = xmlFactory
						.createEnglishLanguageScoreTp();
				essayOrSpeakingScoreTp.setName(LanguageBandScoreTp.ESSAY);
				essayOrSpeakingScoreTp.setScore(languageQualifications
						.getWritingScore().replace(".0", ""));
			} else {
				essayOrSpeakingScoreTp = xmlFactory
						.createEnglishLanguageScoreTp();
				essayOrSpeakingScoreTp.setName(LanguageBandScoreTp.SPEAKING);
				essayOrSpeakingScoreTp.setScore(languageQualifications
						.getSpeakingScore().replace(".0", ""));
			}

			EnglishLanguageScoreTp listeningScoreTp = xmlFactory
					.createEnglishLanguageScoreTp();
			listeningScoreTp.setName(LanguageBandScoreTp.LISTENING);
			listeningScoreTp.setScore(languageQualifications
					.getListeningScore().replace(".0", ""));

			englishLanguageTp.getLanguageScore().addAll(
					Arrays.asList(overallScoreTp, readingScoreTp,
							writingScoreTp, essayOrSpeakingScoreTp,
							listeningScoreTp));
			englishLanguageQualificationDetailsTp
					.getEnglishLanguageQualification().add(englishLanguageTp);
		}
	...
	private NameTp buildAgreedSupervisorName() {
		if (applicationForm.getLatestApprovalRound() != null) {
			ApprovalRound approvalRound = applicationForm
					.getLatestApprovalRound();
