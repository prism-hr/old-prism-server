...
CREATE DEFINER=`pgadmissions`@`%` PROCEDURE `SP_MERGE_USER`(IN `in_merge_from_user_id` INT, IN `in_merge_into_user_id` INT)
	LANGUAGE SQL
	NOT DETERMINISTIC
	CONTAINS SQL
...
BEGIN
	...
	START TRANSACTION;
		...
		INSERT INTO USER_UNUSED_EMAIL (user_id, email)
			SELECT id, email
			FROM USER
			WHERE id = in_merge_from_user_id;
