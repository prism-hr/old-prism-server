...
(SELECT COUNTRIES.ID AS COUNTRIES_ID,DOMICILE.ID AS DOMICILE_ID FROM DOMICILE JOIN COUNTRIES ON DOMICILE.CODE=COUNTRIES.CODE AND DOMICILE.ENABLED='1' AND COUNTRIES.ENABLED='1')
;
/*replace 'French West Indies' with 'St Martin (French Part)'*/
insert into COUNTRIES_DOMICILE select COUNTRIES.ID,DOMICILE.ID from DOMICILE join COUNTRIES where DOMICILE.code='MF' AND COUNTRIES.CODE='XW' AND DOMICILE.ENABLED='1' AND COUNTRIES.ENABLED='1'
;
/*'Eritrea' is not found using country code matching, making a special case for it*/
insert into COUNTRIES_DOMICILE select COUNTRIES.ID,DOMICILE.ID from DOMICILE join COUNTRIES where DOMICILE.name='Eritrea' AND COUNTRIES.NAME='Eritrea' AND DOMICILE.ENABLED='1' AND COUNTRIES.ENABLED='1'
;
