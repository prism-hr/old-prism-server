...
public class ApprovalControllerTest {
	...
	public void shouldGetApprovalPage() {
		...
		controller = new ApprovalController(applicationServiceMock, userServiceMock, approvalServiceMock, approvalRoundValidatorMock,
		                ...
		                scoreFactoryMock, accessServiceMock) {
			@Override
	...
	public void shouldGetApplicantNominatedSupervisors(){
		...
		ProgrammeDetails programmeDetails = new ProgrammeDetailsBuilder().suggestedSupervisors(applicantNominatedSupervisor1,applicantNominatedSupervisor2).build();
	...
	public void shouldGetListOfPreviousSupervisorsAndAddReviewersWillingToApprovalRoundWitDefaultSupervisorsAndApplicantSupervisorsRemoved() {
		...
		final ApplicationForm applicationForm = new ApplicationFormBuilder().id(5).program(program).comments(interviewOne, interviewTwo, interviewThree).programmeDetails(programmeDetails)
		                .build();
		controller = new ApprovalController(applicationServiceMock, userServiceMock, approvalServiceMock, approvalRoundValidatorMock,
		                supervisorPropertyEditorMock, documentPropertyEditorMock, commentValidatorMock, refereesAdminEditDTOValidatorMock,
		                qualificationServiceMock, refereeServiceMock, encryptionHelperMock, sendToPorticoDataDTOEditorMock, sendToPorticoDataDTOValidatorMock,
		...
		EasyMock.expect(userServiceMock.getUserByEmailIncludingDisabledAccounts(emailOfSupervisor1)).andReturn(defaultSupervisor).times(3);
		EasyMock.expect(userServiceMock.getUserByEmailIncludingDisabledAccounts(emailOfSupervisor2)).andReturn(interviewerWillingToSuperviseOne).times(3);
		EasyMock.replay(userServiceMock);
    ...
    public void shouldReturnRefereesAdminEditDTO() throws Exception {
        ...
        EasyMock.replay(currentUserMock, applicationServiceMock, scoringDefinitionParserMock, scoreFactoryMock);
        RefereesAdminEditDTO dto = controller.getRefereesAdminEditDTO("1");
        EasyMock.verify(currentUserMock, applicationServiceMock, scoringDefinitionParserMock, scoreFactoryMock);
