...
public class InstitutionCreator implements ResourceCreator {
    ...
    public Resource create(User user, ResourceDTO newResource) throws Exception {
        ...
        Institution institution = new Institution().withUser(user).withParentResource(system).withDomicile(advert.getAddress().getDomicile())
                ...
                .withGoogleId(advert.getAddress().getGoogleId()).withUclInstitution(false).withCreatedTimestamp(new DateTime()).withLogoImage(logoImage);
