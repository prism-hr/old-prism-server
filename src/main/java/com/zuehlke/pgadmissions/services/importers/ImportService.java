...
/**
 * This is UCL data import service. User for situations where we pull data from UCL system (PORTICO). Currently this is just a set of dictionaries we share.
 * 
 * todo: consider changing name of this class to UclImportService (to achieve consistency with similar UclExportService) todo: consider moving to the parent
 * package
 */
@Service
public class ImportService {
    ...
    private <T extends ImportedObject> void disableElement(T currentElement, List<T> changes) {
        if (currentElement.getEnabled()) {
            ...
            currentElement.setDisabledDate(new Date());
            changes.add(currentElement);
        }
    }
