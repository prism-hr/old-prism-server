...
SET APPLICATION_FORM.next_status = 
	CASE
		WHEN STATECHANGE_COMMENT.next_status = "REVIEW" 
			AND (REVIEW_ROUND.id IS NULL 
				OR REVIEW_ROUND.created_date < LATEST_STATECHANGE_COMMENT.created_timestamp) THEN
			STATECHANGE_COMMENT.next_status
		WHEN STATECHANGE_COMMENT.next_status = "INTERVIEW"
			AND (INTERVIEW.id IS NULL 
				OR INTERVIEW.created_date < LATEST_STATECHANGE_COMMENT.created_timestamp) THEN
			STATECHANGE_COMMENT.next_status
		WHEN STATECHANGE_COMMENT.next_status = "APPROVAL" 
			AND (APPROVAL_ROUND.id IS NULL 
				OR APPROVAL_ROUND.created_date < LATEST_STATECHANGE_COMMENT.created_timestamp) THEN
			STATECHANGE_COMMENT.next_status
	END
WHERE APPLICATION_FORM.status IN ("VALIDATION", "REVIEW", "INTERVIEW", "APPROVAL")
	AND STATECHANGE_COMMENT.next_status IS NOT NULL;
;
...
CREATE PROCEDURE DELETE_APPLICATION_FORM_ACTIONS(IN in_application_form_id INT(10) UNSIGNED)
BEGIN
