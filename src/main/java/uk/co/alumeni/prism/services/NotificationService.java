...
public class NotificationService {
    ...
    public void sendIndividualWorkflowNotifications(Resource resource, Comment comment, Set<UserNotificationDefinitionDTO> updates) {
        Set<User> exclusions = sendIndividualRequestNotifications(resource, comment, updates);
    ...
    public Set<User> sendIndividualRequestNotifications(Resource resource, Comment comment, Set<UserNotificationDefinitionDTO> updates) {
        ...
        if (requests.size() > 0) {
            ...
            List<PrismNotificationDefinition> updateDefinitions = updates.stream().map(UserNotificationDefinitionDTO::getNotificationDefinitionId).collect(Collectors.toList());
            
            for (UserNotificationDefinitionDTO request : requests) {
                PrismNotificationDefinition notificationDefinition = request.getNotificationDefinitionId();
                if (!(notificationDefinition.equals(PrismNotificationDefinition.APPLICATION_PROVIDE_REFERENCE_REQUEST) &&
                        updateDefinitions.contains(PrismNotificationDefinition.APPLICATION_PROVIDE_REFERENCE_NOTIFICATION))) {
                    User recipient = userService.getById(request.getUserId());
                    Long recentRequestCount = recentRequests
                            .get(new UserNotificationDTO().withUserId(request.getUserId()).withNotificationDefinitionId(notificationDefinition));
                    
                    NotificationEvent notificationEvent = new NotificationEvent(this).withNotificationDefinition(notificationDefinition)
