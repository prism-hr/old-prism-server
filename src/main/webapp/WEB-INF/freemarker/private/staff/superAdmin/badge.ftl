...
<form id="applyForm" action="${host}<@spring.url '/apply/new'/>" method="post" target="_blank">
	...
	<button style="border: none; padding: 0" type="submit"<#if RequestParameters.disable??> disabled="disabled"</#if>><img border="0" src="${host}<@spring.url '/design/default/images/prism_apply_now.jpg'/>" /></button>
</form>
