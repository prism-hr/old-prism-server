...
public class ResourceListConstraintBuilder extends ConstraintBuilder {
    ...
    public static <T extends Resource> DetachedCriteria getVisibleResourcesCriteria(User user, Class<T> resourceClass, List<Integer> assignedResources,
        ...
        DetachedCriteria criteria = DetachedCriteria.forClass(resourceClass) //
                ...
                .add(Restrictions.in("id", assignedResources));
            ...
            if (FilterProperty.isPermittedFilterProperty(PrismScope.getResourceScope(resourceClass), property)) {
                ...
                case CLOSING_DATE:
                    ResourceListConstraintBuilder.appendClosingDateFilterCriterion(resourceClass, conditions, propertyName,
                            constraintDTO.getFilterExpression(), constraintDTO.getValueDateStart(), constraintDTO.getValueDateClose(), negated);
                    break;
                case CODE:
                case REFERRER:
                    ResourceListConstraintBuilder.appendStringFilterCriterion(conditions, propertyName, constraintDTO.getValueString(), negated);
                    break;
                ...
                case RATING:
                    ...
                    break;
                case STATE_GROUP:
                    ResourceListConstraintBuilder.appendStateGroupFilterCriterion(conditions, propertyName, constraintDTO.getValueStateGroup(), negated);
                    break;
