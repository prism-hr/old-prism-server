...
public class ProjectConfigurationControllerTest {
	...
	public void setup() throws IOException {
		...
		programsService = EasyMock
				.createMock(ProgramsService.class);
		ApplicationContext applicationContext = EasyMock
				.createMock(ApplicationContext.class);
		ProjectDTOValidator projectDTOValidator = EasyMock
				.createMock(ProjectDTOValidator.class);
		DatePropertyEditor datePropertyEditor = EasyMock
				.createMock(DatePropertyEditor.class);
		ProgramPropertyEditor programPropertyEditor = EasyMock
				.createMock(ProgramPropertyEditor.class);
		PersonPropertyEditor personPropertyEditor = EasyMock
				.createMock(PersonPropertyEditor.class);
		projectConverter = EasyMock.createMock(ProjectConverter.class);
		ApplyTemplateRenderer templateRenderer = EasyMock
				.createMock(ApplyTemplateRenderer.class);
		String host = "localhost";
		controller = new ProjectConfigurationController(userService,
				programsService, applicationContext, projectDTOValidator,
				datePropertyEditor, programPropertyEditor,
				personPropertyEditor, projectConverter, templateRenderer, host);
	...
	private Project createProject() {
		...
		RegisteredUser primarySupervisor = new RegisteredUserBuilder()
				.firstName("Ezio").lastName("Imbecilo").email("ezio@mail.com")
				.id(1).build();
		RegisteredUser secondarySupervisor = new RegisteredUserBuilder()
				.firstName("Genowefa").lastName("Pigwa")
				.email("gienia@mail.com").id(2).build();
		Project project = new ProjectBuilder().id(1).advert(advert)
				.program(program).primarySupervisor(primarySupervisor)
